...
def main():
    parser = argparse.ArgumentParser(
        description='JV dataset filelists generator')
    parser.add_argument('transcripts', type=Path, default='./line_index.tsv',
    ...
    # Define val and test; the remaining ones will be train IDs using train_test_split
    ...
    prefix = Path(args.output_dir, "jv_fe_audio_pitch_text_")
    ...
    prefix = Path(args.output_dir, "jv_fe_audio_")
    ...
    generate(Path(args.output_dir, "jv_fe_audio_text.txt"),
             {**val_ids_text, **test_ids_text, **train_ids_text}, False, True)

    generate_inference_tsv(Path(args.output_dir, "jv_fe_test.tsv"), test_ids_text)
